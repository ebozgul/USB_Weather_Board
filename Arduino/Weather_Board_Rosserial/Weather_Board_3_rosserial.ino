...
#include <ros.h>  // Ros-serial libraries
#include <std_msgs/String.h>

// Rosserial: Instantiate NodeHandle
ros::NodeHandle nh;
// Rosserial: Instantiate publishers and subscribers
std_msgs::String str_msg;
ros::Publisher wb_publisher("wb_publisher", &str_msg); // publisher name: "wb_publisher"
...
// perform all the settings we need before running the main loop
{
  	...
  	// Initialie Ros node handle
	nh.initNode();
	nh.advertise(wb_publisher);
...
// loops forever after setup() ends
{
  ...
  batt_volts = float(analogRead(BATT_LVL)) / BATT_RATIO;
	...
	// RosSerial Msg Publishing
	char charBuf[13];
        String comma = ", ";
        String total;
	dtostrf(SHT15_temp, 2 , 1, charBuf);
	total = charBuf;
        dtostrf(SHT15_humidity, 2 , 1, charBuf);
        total.concat(comma);
        total.concat(charBuf);
        dtostrf(SHT15_dewpoint, 2 , 1, charBuf);
        total.concat(comma);
        total.concat(charBuf);
        dtostrf(BMP085_pressure, 2 , 4, charBuf);
        total.concat(comma);
        total.concat(charBuf);
        dtostrf(BMP085_temp, 2 , 1, charBuf);
        total.concat(comma);
        total.concat(charBuf);
        dtostrf(TEMT6000_light, 2 , 1, charBuf);
        total.concat(comma);
        total.concat(charBuf);
          
          total.toCharArray(charBuf, 50);
	str_msg.data = charBuf;
        wb_publisher.publish( &str_msg );
	nh.spinOnce();
